...
from ..core.types import PowerOf3Signal, TradingSession, SignalType, SignalQuality
...
# Removed unresolved import "data.providers.data_provider"
...
# Ensure yfinance is always imported
...
class BacktestResults:
    ...
    def to_dict(self) -> Dict:
        ...
        result['trades'] = [trade.to_dict() for trade in self.trades or []]
        return result
...
class FallbackDataProvider:
    ...
    def get_historical_data(symbol: str, start_date: str, end_date: str, 
        ...
        if not DATA_PROVIDER_AVAILABLE:
            import yfinance as yf  # Ensure yfinance is imported locally if needed
        try:
...
class PowerOf3BacktestEngine:
    ...
    async def run_backtest(self, symbol: str, start_date: str, end_date: str,
        ...
        # Get historical data
        df = await self._get_historical_data(symbol, start_date, end_date, timeframe)
    ...
    async def _get_historical_data(self, symbol: str, start_date: str, 
    ...
    def _calculate_results(self, symbol: str, start_date: str, end_date: str) -> BacktestResults:
        ...
        if len(self.closed_trades) > 1:
            ...
            sharpe_ratio = float(np.mean(returns)) / float(np.std(returns)) if np.std(returns) > 0 else 0.0
        else:
            sharpe_ratio = 0
        ...
        avg_trade_duration = timedelta(seconds=np.mean(durations).total_seconds()) if durations else timedelta()
        ...
        return BacktestResults(
            ...
            avg_trade_duration=timedelta(seconds=avg_trade_duration.total_seconds()),
