...
import requests
from main import PowerOf3SignalTool
...
#             self.signal_generator = PowerOf3SignalGenerator(min_risk_reward=5.0, max_risk_percent=2.0)
#         else:
...
#             return """Power of 3 Signal Generator not available.
...
#                 return "Power of 3 Signal Generator is not available. Please ensure the required module is correctly imported."
            
#             signals = self.signal_generator.generate_signals(symbol, df, account_balance)
            
#             if not signals:
#                 # Provide analysis even if no signals
#                 current_session = self.signal_generator.session_manager.get_current_session()
#                 session_status = f"Current session: {current_session.value if current_session else 'None (outside Power of 3 hours)'}"
                
#                 # Get basic analysis
#                 liquidity_zones = self.signal_generator.liquidity_analyzer.identify_liquidity_zones(
#                     df, current_session or TradingSession.NEW_YORK_OPEN
#                 )
                
#                 manipulation_patterns = self.signal_generator.manipulation_detector.detect_manipulation_patterns(
#                     df, current_session or TradingSession.NEW_YORK_OPEN, liquidity_zones
#                 )
                
#                 market_structure = self.signal_generator._analyze_market_structure(df)
...
def create_market_analyst_agent() -> Agent:
    ...
    return Agent(
        ...
        max_iter=3,  # Limit iterations to avoid loops
    )
...
def create_signal_generator_agent() -> Agent:
    ...
    return Agent(
        ...
        tools=[PowerOf3SignalTool(), TechnicalAnalysisTool()],
        verbose=True,
        allow_delegation=False,
        llm=ChatOpenAI(model="gpt-4", temperature=0.1),
        max_iter=2
    ...
    if standalone_passed:
        ...
        if basic_test_passed:
            ...
            if not trading_system.crew:
                ...
                print("✓ System is ready for Power of 3 trading analysis!")
                print("✓ Use the power_of_3_signal_tool for real-time signal generation")
                print("✓ Monitor during London Open (2-5 AM EST) and NY Open (7-10 AM EST)")
                print("✓ Look for manipulation patterns and liquidity sweeps")
